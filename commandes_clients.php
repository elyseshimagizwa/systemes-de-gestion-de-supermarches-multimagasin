...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['statut'], $_POST['commande_id'])) {
    ...
    $orderId = (int)$_POST['commande_id'];
    $check = $pdo->prepare('SELECT magasin_id FROM commandes_clients WHERE id=?');
    $check->execute([$orderId]);
    $orderStore = (int)$check->fetchColumn();
    if ($isAdmin || $orderStore === (int)currentMagasinId()) {
        try {
            $updated = updateClientOrderStatus($pdo, $orderId, $status, (int)$user['id']);
            sendClientOrderEmail($updated, $status, getSettings());
...
$sql = "SELECT cc.*, u.nom AS client_nom, u.email AS client_email, m.nom AS magasin_nom FROM commandes_clients cc JOIN utilisateurs u ON u.id=cc.utilisateur_id JOIN magasins m ON m.id=cc.magasin_id";
$params = [];
if (!$isAdmin) { $sql .= ' WHERE cc.magasin_id=?'; $params[] = (int)currentMagasinId(); }
$sql .= ' ORDER BY cc.date_commande DESC';
